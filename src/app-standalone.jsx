...
function SplashScreen({ exiting }) {
  return (
    <div style={{
      ...
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=Fraunces:wght@600;700&family=Inter:wght@500;600&display=swap');
        @keyframes ff-pulse {
        ...
        .ff-splash-mark { animation: ff-pulse 1.6s ease-in-out infinite; }
        .ff-splash-text { animation: ff-fade-in 0.6s ease 0.15s both; }
      `}</style>
...
function FocusFlow() {
  ...
  React.useEffect(() => {
    ...
    const safetyTimer = setTimeout(revealDashboard, 1000);

    const elapsed = Date.now() - startedAt;
    const remaining = Math.max(0, MIN_SPLASH_MS - elapsed);
    const revealTimer = setTimeout(revealDashboard, remaining);
  ...
  // The dashboard itself (once data has arrived) renders here, underneath
  // the splash overlay — fully built and ready, but hidden behind the
  // opaque splash on top. Only once it's actually ready do we fade the
  // splash away, so the reveal is a clean cut to a complete screen rather
  // than two separate fades that both dip through a near-blank moment.
  const dashboardTree = !loading && (
